...
# Function that calculates the backward algorithm matrix on the input sequence, following the HMM
def doBackward(seq, transProb, emissProb):
    ...
    for i in range(4):
        myMatrix[i, columnNum] = 1
    
    columnNum -= 1
    
    while columnNum >= 0:

        symbIndex = getSymb(seq, columnNum)

        for k in range(1, 4):   
            
            zeroStateVal = transProb[k, 0] * emissProb[0, symbIndex] * myMatrix[0, columnNum + 1]
            firstStateVal = transProb[k, 1] * emissProb[1, symbIndex] * myMatrix[1, columnNum + 1]
            secondStateVal = transProb[k, 2] * emissProb[2, symbIndex] * myMatrix[2, columnNum + 1]
            thirdStateVal = transProb[k, 3] * emissProb[3, symbIndex] * myMatrix[3, columnNum + 1]

            myMatrix[k, columnNum] = firstStateVal + secondStateVal + thirdStateVal
        
        columnNum -= 1

    return myMatrix

# Function that applies the forward algorithm on the input sequence, following the HMM
...
def main():
    ...
    # Calculate the probability of the input sequence. Also return the forward matrix
    probOfSequence, forwardMatrix = calcProbSequence(sequence, transProb, emissProb)

    # Calculate the backward matrix
    backwardMatrix = doBackward(sequence, transProb, emissProb)

    # Formula to get a matrix with the posterior probabilities
    posteriorProb = (forwardMatrix * backwardMatrix) / probOfSequence
    ...
    print("The posterior probabilities matrix is:\n", posteriorProb)
